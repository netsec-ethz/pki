...
class ConfElem(object):
    def __init__(isd_as, ip, pubkey):
        self.addr = SCIONAddr.from_values(ISD_AS(isd_as), haddr_parse(1, ip))
        self.pubkey = pubkey


class Conf(object):
    def __init__(conf_file, privkey_file=None, elem_id=None):
        self.my_id = elem_id
